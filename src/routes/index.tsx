...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "The Museum of Shalmalee — a curated portfolio of UI/UX developer Shalmalee Raje." },
    ],
  }),
  component: Portfolio,
});
...
function Cursor() {
  ...
  useEffect(() => {
    ...
    const over = (e: MouseEvent) => {
      ...
      if (el) { setHover(true); setLabel(el.dataset.cursor || null); }
  ...
  return (
    <motion.div style={{ x, y }} className="pointer-events-none fixed left-0 top-0 z-[100] hidden md:block" aria-hidden>
      <motion.div
        animate={{ scale: hover ? 3.2 : 1, backgroundColor: hover ? "var(--accent)" : "var(--primary)" }}
...
function Portfolio() {
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground">
      <Cursor />
      <motion.div style={{ width: progressX }} className="fixed left-0 top-0 z-[60] h-[3px] bg-gradient-to-r from-primary via-accent to-secondary" />
      <Nav />
      <Hero parallax={heroParallax} />
      <Marquee items={["Room I · Foyer", "Room II · Practice", "Room III · Exhibits", "Room IV · Correspondence"]} />
      <About />
      <Skills />
      <Projects />
      <Contact />
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 md:px-10">
        <a href="#top" data-cursor="entrance" className="flex items-center gap-3 uppercase tracking-[0.2em] text-sm font-bold">
          <span className="inline-block size-2 rounded-full bg-accent" />
          The Museum of Shalmalee
        </a>
        <nav className="hidden items-center gap-8 text-xs uppercase tracking-[0.25em] md:flex">
          {[["I. Foyer","#about"],["II. Practice","#skills"],["III. Exhibits","#work"],["IV. Contact","#contact"]].map(([l,h]) => (
...
function Hero({ parallax }: { parallax: any }) {
  return (
    <section id="top" className="relative grain overflow-hidden pt-36 pb-20 md:pt-44 md:pb-32">
      <motion.img
        src={heroBlobs} alt="Starry night inspired backdrop" style={{ y: parallax }}
      ...
      <div className="relative mx-auto max-w-7xl px-6 md:px-10">
        <Reveal>
          <p className="mb-6 flex items-center gap-3 text-xs uppercase tracking-[0.35em] text-muted-foreground">
            <span className="h-px w-12 bg-foreground/40" /> Now Open · Est. 2026 · Free Admission
          </p>
        </Reveal>
        ...
        <div className="mt-10 grid gap-10 md:grid-cols-12">
          <Reveal delay={0.3} className="md:col-span-7">
            <p className="text-lg text-balance md:text-2xl leading-snug">
              UI/UX Developer 
              I curate <em className="text-accent not-italic font-semibold">interfaces like exhibits</em>:
              researched, arranged with intent, and framed for the person walking through them.
            </p>
          </Reveal>
          <Reveal delay={0.5} className="md:col-span-4 md:col-start-9 flex flex-col items-start gap-2 self-end text-sm">
            <span className="text-xs uppercase tracking-[0.25em] text-muted-foreground">Museum Location</span>
            <span className="text-xl font-semibold">Pune, Maharashtra 🇮🇳</span>
            <a href="mailto:shalmaleeraje1028@gmail.com" data-cursor="email" className="mt-3 underline-offset-4 hover:underline">
        ...
        <div className="mt-16 flex flex-wrap items-center gap-4">
          <Magnetic>
            <a href="#work" data-cursor="tour" className="group inline-flex items-center gap-3 rounded-full bg-primary px-7 py-4 text-primary-foreground text-sm uppercase tracking-[0.2em] transition-shadow hover:shadow-[var(--shadow-soft)]">
              Begin the tour
              <span className="grid size-6 place-items-center rounded-full bg-primary-foreground/15 transition-transform group-hover:translate-x-1">→</span>
            </a>
          </Magnetic>
          <Magnetic>
            <a href="#contact" data-cursor="say hi" className="rounded-full border border-foreground/30 px-7 py-4 text-sm uppercase tracking-[0.2em] transition-colors hover:border-foreground hover:bg-foreground hover:text-background">
              Guestbook
            </a>
          </Magnetic>
        </div>
...
function RoomHeader({ n, title, sub }: { n: string; title: string; sub?: string }) {
  return (
    <div className="mb-14 flex items-center gap-6">
      <div className="grid size-14 place-items-center rounded-full border border-foreground text-sm font-bold">{n}</div>
      <div>
        <p className="text-xs uppercase tracking-[0.4em] text-muted-foreground">Room {n}</p>
        <h2 className="mt-1 text-3xl font-bold uppercase tracking-tight md:text-5xl">{title}</h2>
...
function About() {
  return (
    <section id="about" className="relative mx-auto max-w-7xl px-6 py-24 md:px-10 md:py-32">
      <RoomHeader n="I" title="The Foyer" sub="An introduction to the curator" />
      <div className="grid gap-12 md:grid-cols-12">
        <Reveal className="md:col-span-8">
          <p className="text-2xl leading-snug md:text-4xl font-light">
            I'm passionate about UI/UX because it lets me <span className="text-primary font-semibold">understand people</span> beyond the screen —
            researching behavior, finding quiet pain points, and shaping digital spaces that feel
            <span className="text-accent font-semibold"> simple, meaningful, and human</span>.
          </p>
        </Reveal>
        <Reveal delay={0.2} className="md:col-span-4 border-l border-foreground/15 pl-6">
          <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground">Wall Label</p>
          <p className="mt-3 text-sm leading-relaxed">
            Shalmalee Raje (b. Pune) is a UI/UX developer working at the intersection of research, interface and
            code — treating every product as a small exhibition.
          </p>
        </Reveal>
      </div>
      ...
      <Reveal delay={0.2}>
        <div className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            <div key={l} className="rounded-2xl border border-foreground/20 bg-card p-5">
...
function Skills() {
  ...
  const soft = ["User Research", "Empathy-driven design", "Design Thinking", "Prototyping", "Interaction Design", "Visual Storytelling"];

  return (
    <section id="skills" className="relative bg-foreground text-background grain">
      <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 md:py-32">
        <div className="mb-14 flex items-center gap-6">
          <div className="grid size-14 place-items-center rounded-full border border-background text-sm font-bold">II</div>
          <div>
            <p className="text-xs uppercase tracking-[0.4em] opacity-60">Room II</p>
            <h2 className="mt-1 text-3xl font-bold uppercase tracking-tight md:text-5xl">The Practice Room</h2>
        ...
        <div className="grid gap-16 md:grid-cols-12">
          <div className="md:col-span-7 space-y-7">
            <p className="text-xs uppercase tracking-[0.3em] opacity-60">Technical Proficiency</p>
            {tech.map(([name, pct], i) => (
          ...
          <div className="md:col-span-5">
            <p className="text-xs uppercase tracking-[0.3em] opacity-60">Craft & Approach</p>
            <div className="mt-6 flex flex-wrap gap-3">
              {soft.map((s) => (
                <Magnetic key={s}>
                  <span data-cursor="•" className="inline-flex cursor-pointer rounded-full border border-background/40 px-4 py-2 text-sm uppercase tracking-[0.15em] transition-colors hover:bg-background hover:text-foreground">
                    {s}
                  </span>
                </Magnetic>
              ))}
  ...
  return (
    <div>
      <div className="mb-2 flex items-baseline justify-between">
        <span className="text-lg font-semibold uppercase tracking-[0.1em]">{name}</span>
        <motion.span
...
function Projects() {
  return (
    <section id="work" className="relative mx-auto max-w-7xl px-6 py-24 md:px-10 md:py-32">
      <RoomHeader n="III" title="The Exhibits" sub="Selected works, hung chronologically" />
...
function Exhibit({ exhibit, flip }: { exhibit: Exhibit; flip: boolean }) {
  return (
    <Reveal>
      <article className={`grid gap-8 md:grid-cols-12 md:items-center ${flip ? "md:[&>*:first-child]:order-2" : ""}`}>
        {/* Frame */}
        <div className="md:col-span-8">
          <Tilt>
            <a
              href={exhibit.href} target="_blank" rel="noreferrer" data-cursor="visit ↗"
            ...
            >
              <div className="flex items-center justify-between border-b border-foreground/10 bg-background/60 px-4 py-2 text-xs">
                ...
                <span className="rounded-full bg-foreground/5 px-3 py-1 font-mono">{new URL(exhibit.href).host}</span>
                <span className="opacity-50">live ↗</span>
              </div>
              <div className="relative aspect-[16/10] overflow-hidden bg-background">
                {exhibit.thumb ? (
                  <img
                    src={exhibit.thumb} alt={exhibit.title}
                    className="absolute inset-0 h-full w-full object-cover object-top transition-transform duration-700 group-hover:scale-[1.04]"
                    loading="lazy"
                  />
                ) : exhibit.liveEmbed ? (
                  <iframe
                    src={exhibit.href} title={exhibit.title} loading="lazy"
                  ...
                  <div className="absolute inset-0 grid place-items-center bg-gradient-to-br from-primary to-accent">
                    <span className="text-6xl font-bold uppercase tracking-tight text-background">{exhibit.title}</span>
                  </div>
                )}
                <div className="absolute inset-0 flex items-end justify-between bg-gradient-to-t from-foreground/40 via-transparent to-transparent p-6 opacity-0 transition-opacity group-hover:opacity-100">
                  <p className="text-2xl font-bold uppercase text-background">View the exhibit</p>
                  <span className="rounded-full bg-background px-5 py-2.5 text-xs uppercase tracking-[0.2em] text-foreground">Open ↗</span>
                </div>
              </div>
            </a>
          </Tilt>
        ...
        <div className="md:col-span-4">
          <div className="border-l-4 border-accent bg-card p-6 shadow-sm">
            <p className="text-xs font-mono uppercase tracking-[0.3em] text-muted-foreground">Exhibit № {exhibit.no}</p>
            <h3 className="mt-3 text-3xl font-bold uppercase leading-tight">{exhibit.title}</h3>
            <p className="mt-3 text-sm"><span className="text-muted-foreground">Role: </span>{exhibit.role}</p>
            <p className="text-sm"><span className="text-muted-foreground">Year: </span>{exhibit.year}</p>
            <p className="text-sm"><span className="text-muted-foreground">Medium: </span>{exhibit.medium}</p>
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{exhibit.desc}</p>
            <a
              href={exhibit.href} target="_blank" rel="noreferrer" data-cursor="visit"
...
function Contact() {
  return (
    <section id="contact" className="relative overflow-hidden bg-primary text-primary-foreground grain">
      <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 md:py-32">
        <div className="mb-14 flex items-center gap-6">
          <div className="grid size-14 place-items-center rounded-full border border-primary-foreground text-sm font-bold">IV</div>
          <div>
            <p className="text-xs uppercase tracking-[0.4em] opacity-70">Room IV</p>
            <h2 className="mt-1 text-3xl font-bold uppercase tracking-tight md:text-5xl">The Guestbook</h2>
          </div>
        </div>
        ...
        <div className="mt-12 grid gap-10 md:grid-cols-12">
          <Reveal className="md:col-span-7">
            <Magnetic>
              <a href="mailto:shalmaleeraje1028@gmail.com" data-cursor="email"
                ...
                <span className="text-xl font-semibold">shalmaleeraje1028@gmail.com</span>
                <span className="grid size-8 place-items-center rounded-full bg-accent-foreground/10">↗</span>
              </a>
            </Magnetic>
          </Reveal>
  ...
  return (
    <Reveal>
      <div className="flex items-center justify-between border-b border-primary-foreground/15 pb-3">
        <span className="text-xs uppercase tracking-[0.25em] opacity-60">{k}</span>
  ...
  return (
    <footer className="border-t border-foreground/10 bg-background">
      <div className="mx-auto flex max-w-7xl flex-col items-start gap-4 px-6 py-10 md:flex-row md:items-center md:justify-between md:px-10">
        <p className="text-sm uppercase tracking-[0.25em] font-bold">Shalmalee Raje · UI/UX Developer</p>
        <p className="text-xs uppercase tracking-[0.25em] text-muted-foreground">© 2026 — The Museum closes at midnight.</p>
      </div>
    </footer>
  );
}
